...
describe('AdvertiserSignupForm', () => {
  ...
  const fillCompanyStep = async (user: ReturnType<typeof userEvent.setup>) => {
  ...
  it("ouvre directement l'étape entreprise sans proposer de type de compte", () => {
    render(<AdvertiserSignupForm />)

    expect(screen.getByText(/Informations sur votre entreprise/i)).toBeInTheDocument()
    expect(screen.queryByText(/Quel type de compte souhaitez-vous créer/i)).not.toBeInTheDocument()
    expect(screen.queryByRole('button', { name: /Personne physique/i })).not.toBeInTheDocument()
  })
  ...
  it("n'affiche pas d'erreur au simple changement de focus", async () => {
    ...
    const companyNameInput = screen.getByLabelText(/Nom de l'organisme/i)
    await user.click(companyNameInput)
    await user.tab()

    expect(screen.queryByText(/Le nom de l'organisme est obligatoire/i)).not.toBeInTheDocument()
  })

  it("empêche de passer à l'étape compte si le téléphone du représentant légal manque", async () => {
    render(<AdvertiserSignupForm />)
    const user = userEvent.setup()
  ...
  it("empêche la création du compte tant que l'étape 2 est incomplète", async () => {
    render(<AdvertiserSignupForm />)
    const user = userEvent.setup()
  ...
  it("demande une confirmation du mot de passe à l'étape compte", async () => {
    render(<AdvertiserSignupForm />)
    const user = userEvent.setup()
  ...
  it("bloque l'inscription annonceur si l'e-mail existe déjà sur un compte comédien", async () => {
    mockSupabase.from.mockImplementation((table: string) => {
      ...
      if (table === 'comediens') {
        return {
          select: jest.fn().mockReturnValue(createProfileSelectChain({ id: 'comedien-1' })),
          insert: jest.fn(),
        }
      }
    ...
    await user.type(screen.getByLabelText(/Adresse e-mail/i), 'comedien@sceno.fr')
    await user.type(screen.getByLabelText(/^Mot de passe/i), 'Password123')
    ...
    expect(mockSupabase.auth.signUp).not.toHaveBeenCalled()
    expect(screen.getByText(/Un compte comédien existe déjà avec cet email/i)).toBeInTheDocument()
  })
  ...
  it("affiche un message clair quand Supabase limite les e-mails de confirmation annonceur", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Trop de tentatives d'envoi d'e-mails ont été détectées/i)
      ).toBeInTheDocument()
    })
  }, 10000)
})
